...
def binarize_cpu(W):
    A = numpy.sum(W, axis=(1,2,3))
    A = A[:, numpy.newaxis, numpy.newaxis, numpy.newaxis]
    Wb = numpy.where(W>=0, 1, -1).astype(numpy.float32, copy=False)
    return Wb*A

def binarize_gpu(W):
    A = cuda.cupy.sum(W, axis=(1,2,3))
    A = A[:, cuda.cupy.newaxis, cuda.cupy.newaxis, cuda.cupy.newaxis]
    Wb = _kern()(W)
    return Wb*A

class BinaryXnorConvolution2DFunction(function.Function):
...
def binary_xnor_convolution_2d(x, W, b=None, stride=1, pad=0, use_cudnn=True):
    ...
    """
    func = BinaryXnorConvolution2DFunction(stride, pad, use_cudnn)
    if b is None:
        return func(x, W)
    else:
        return func(x, W, b)
